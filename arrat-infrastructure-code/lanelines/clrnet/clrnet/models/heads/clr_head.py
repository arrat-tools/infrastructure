...
class CLRHead(nn.Module):
    ...
    def predictions_to_pred(self, predictions,scores):
        '''
        ...
        lanes = []
        scrs = []
        for lane,score in zip(predictions,scores):
    ...
    def get_lanes(self, output, as_lanes=True):
        ...
        for predictions in output:
            ...
            if as_lanes:
                pred,scrs = self.predictions_to_pred(predictions,scores)
            else:
                pred = predictions
            #scr = scores.
            decoded.append(pred)
            confidence.append(scrs)
