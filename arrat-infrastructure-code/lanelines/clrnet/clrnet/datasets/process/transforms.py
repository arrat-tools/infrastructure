...
class CropToShape(object):
    ...
    def __call__(self,img_group):
        ...
        h, w = img_group['img'].shape[0:2]
        h1 = max(0, self.size[1])
        w1 = max(0, self.size[0])
        h2 = min(self.size[3], h)
        w2 = min(self.size[2], w)

        img_group['img'] = img_group['img'][h1:h2, w1:w2, ...]
        return img_group
